      ...
      let req = this.#protocol.request(options, res => {
        ...
        res.on('end', () => {
          if (res.statusCode === 401) {
            reject(new Error(`SBER JSON-RPC: Connection Rejected: 401 Unauthorized`))
          } else if (res.statusCode === 403) {
            reject(new Error(`SBER JSON-RPC: Connection Rejected: 403 Forbidden`))
          } else if (res.statusCode === 500 && buffer === 'Work queue depth exceeded') {
            let exceededError = new Error(`SBER JSON-RPC: ${buffer}`)
            ...
            try {
              ...
              reject(new Error(`SBER JSON-RPC: Error Parsing JSON: ${err.message}`))
            }
          }
        })
      })
      req.on('error', err => reject(new Error(`SBER JSON-RPC: Request Error: ${err.message}`)))
